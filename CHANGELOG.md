...
- Replaced the three-part provision identifier with `AREA.PAGE.TOPIC.NNN`. The old prefix labelled the subject area and served as the global page-uniqueness key at the same time, so 33 baseline prefixes competed in one flat namespace and peer pages lost their natural names. `COMPONENT.OWNERSHIP.001` is now `FRONTEND.COMPONENTS.OWNERSHIP.001` and `BTEST.PROJECTS.001` is now `BACKEND.TESTING.PROJECTS.001`. The grammar is the extension form promoted to the universal rule, so the 329 `EXT.*` provisions keep their shape.
- Renamed the six Blazor client prefixes. `CLIENT`, `CLIENTUI`, `CLIENTDATA`, `CLIENTRENDER`, `CLIENTTEST`, and `BROWSER` existed only because `FRONTEND`, `COMPONENT`, `DATA`, `RENDER`, and `FTEST` were already taken by React. They are now `BLAZOR.STRUCTURE`, `BLAZOR.COMPONENTS`, `BLAZOR.DATA`, `BLAZOR.RENDERING`, `BLAZOR.TESTING`, and `BLAZOR.BROWSER`.
- Declared the eight areas `CORE`, `PLATFORM`, `WORKSPACE`, `BACKEND`, `FRONTEND`, `BLAZOR`, `QUALITY`, and `EXT` in `standards.manifest.json` under `idRegistry`, together with the scope each of the 48 normative pages owns. The validator checks provisions against that declaration instead of inferring prefix width from the page class, and reports `ID_PAGE_UNREGISTERED`, `ID_AREA_UNKNOWN`, and `ID_REGISTRY_STALE`.
...
- Renamed `PROFILE.*` to `PLATFORM.NEXTJS.*`. The generic word belonged to one specific profile, and `AGENTS.md` cited `PROFILE.VERSIONS.001` for manifest version pins even though it is profile-neutral, pointing a Blazor consumer at the Next.js page. `AGENTS.md` now cites both profile provisions.
- Renamed three extension namespaces that disagreed with their page names. `EXT.API` is `EXT.COMPAT`, `EXT.DATA` is `EXT.LIFECYCLE`, and `EXT.EXTERNAL` is `EXT.INTEGRATIONS`. This also removes the `EXT.API` and `API` collision and the `EXT.DATA` and `DATA` collision, where one word named two concepts at two depths.
- Renamed `WRITING.CONVENTION.001` to `CORE.AUTHORING.DEFAULTS.001`. It was a Standard carrying the reserved `CONVENTION` segment, which is the only signal of normative force inside an identifier. `CORE.AUTHORING.IDENTIFIER.002` now prohibits that, and the validator enforces it.
- Gave `UI.PAGE.SPEC.001`, `UI.AGENT.PROTOCOL.001`, and `DEP.FRONTEND.UI.001` single-word topics. They are `FRONTEND.UI.PAGE.001`, `FRONTEND.UI.PROTOCOL.001`, and `WORKSPACE.DEPENDENCIES.UI.001`.
- Collapsed the four-segment convention identifiers. `WRITING.HEADING.CONVENTION.001` and its two siblings are `CORE.AUTHORING.CONVENTION.001` through `003`, and `UI.GOVERNANCE.CONVENTION.*` is `FRONTEND.UI.CONVENTION.*`. A convention ID is now exactly `AREA.PAGE.CONVENTION.NNN`.
- Closed the topic vocabulary. `idRegistry.topics` registers all 245 topic words, and the validator reports `ID_TOPIC_UNKNOWN` for an unregistered word and `ID_TOPIC_DUPLICATE` when one concept carries a singular and a plural form. Fourteen concepts had both spellings, so 36 provisions moved to the singular form, including `ERRORS` to `ERROR`, `STATES` to `STATE`, and `MIGRATIONS` to `MIGRATION`. `EXT.OUTBOX.OPERATIONS` covered backlog indicators and delivery alerts, which is monitoring rather than an operation identifier, so it became `EXT.OUTBOX.OBSERVABILITY`.
- Added `docs/reference/provisions.md`, a generated index resolving every provision ID to its heading, its owning page, and a link to its anchor. There was no lookup table before, and `AGENTS.md` alone cites 46 distinct identifiers. `tools/generate-provisions.mjs` writes it, `tools/provision-index.mjs` builds the content for both the generator and the validator, and `CORE.AUTHORING.INDEX.001` requires the checked-in page to match.
- Corrected the Reference example in the authoring standard. It illustrated the provision contract with `ARCH.DOMAIN.001` and the requirement text `Keep Domain package-free`, while the real provision is `Keep business invariants in Domain` and package references belong to `ARCH.DEPENDENCIES.001`. The validator passed because the identifier existed. The example now quotes the real provision, and the fenced block uses a grammar placeholder.
- Moved `docs/conventions/quality/backend-testing.md` to `docs/conventions/backend/testing.md`. Backend testing sat under quality while frontend testing sat under frontend, and the page now owns `BACKEND.TESTING`.
- Realigned the manifest load plan keys to the same area taxonomy. `repository.writing` is `core.authoring`, `testing.backend` is `backend.testing`, `client.*` is `blazor.*`, and `security.review` is `quality.security`. Added the `blazor.components` and `blazor.browser` plans, which the Blazor pages needed for an agent to select the narrowest task.
- Added `CORE.AUTHORING.IDENTIFIER.001`, `CORE.AUTHORING.IDENTIFIER.002`, `CORE.AUTHORING.IDENTIFIER.003`, and `CORE.AUTHORING.INDEX.001`. The authoring standard states the grammar, the reserved segment, the closed vocabulary, and the derived index as a `Provision identity` concept.
- Tightened the identifier grammar in `tools/validate-standards.mjs` and `schemas/standards-project.schema.json` to exactly four segments with a three-digit sequence. Every citation scanner now shares one pattern requiring that three-digit tail, so a stale three-part identifier still reports as an unknown reference while grammar notation such as `AREA.PAGE.TOPIC.NNN` no longer produces a false positive.
- Moved `standards.manifest.json` to `schemaVersion` 4 for the `idRegistry` and `overridePolicy` fields.
- Added the `Page scope` and `Provision ID` glossary terms and rewrote the `Rule ID` and `Convention ID` entries.
- Grew the validator fixture suite from 84 to 96 cases, covering every new diagnostic with a failing fixture.
