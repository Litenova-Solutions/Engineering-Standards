...
- Recipes live in one catalog, and a page region names one. (standards/rule/frontend-ui.compose-from-a-catalog-recipe, standards/rule/frontend-ui.publish-each-recipe-as-a-page-and-a-sidecar)
- Every registered route carries a schema-valid sidecar. (standards/rule/frontend-ui.specify-pages-before-composition)
- A sidecar names a component for every state it declares. (standards/rule/frontend-ui.render-every-declared-state)
- An implementation renders no region its frozen sidecar omits. (standards/rule/frontend-ui.keep-the-implementation-inside-the-frozen-plan)
- Acceptance files sit beside the route and resolve every named identifier. (standards/rule/frontend-ui.place-acceptance-beside-the-route, standards/rule/frontend-ui.resolve-every-acceptance-identifier)
...
- Agents research, freeze, implement, then verify in a browser. (standards/rule/frontend-ui.follow-the-agent-ui-protocol, standards/rule/frontend-ui.prove-ui-behavior-and-appearance)
...
**Rationale:** A page can then be reviewed without reconstructing the CLI command that produced its components. `schemas/ui-vocabulary.schema.json` is the shape, and `node standards/tools/validate-ui.mjs` is what reads the file against it. The schema requires the full preset only for the default visual system. A frontend on an alternate system records its own baseline and carries the override decision `standards/rule/frontend-ui.select-one-visual-authority` requires.
...
### Publish each recipe as a page and a sidecar (standards/rule/frontend-ui.publish-each-recipe-as-a-page-and-a-sidecar)
...
### Compose from a catalog recipe (standards/rule/frontend-ui.compose-from-a-catalog-recipe)
...
### Specify pages before composition (standards/rule/frontend-ui.specify-pages-before-composition)
...
### Render every declared state (standards/rule/frontend-ui.render-every-declared-state)
...
### Keep the implementation inside the frozen plan (standards/rule/frontend-ui.keep-the-implementation-inside-the-frozen-plan)
...
### Place acceptance beside the route (standards/rule/frontend-ui.place-acceptance-beside-the-route)
...
### Resolve every acceptance identifier (standards/rule/frontend-ui.resolve-every-acceptance-identifier)
...
### Follow the agent UI protocol (standards/rule/frontend-ui.follow-the-agent-ui-protocol)
...
### Promote a recipe on its second consumer (standards/rule/frontend-ui.promote-a-recipe-on-its-second-consumer)
...
| standards/rule/frontend-ui.publish-each-recipe-as-a-page-and-a-sidecar | static | `node standards/tools/validate-ui.mjs` reports a catalog recipe with no sidecar and a sidecar failing `schemas/composition-recipe.schema.json`. |
| standards/rule/frontend-ui.compose-from-a-catalog-recipe | static | `node standards/tools/validate-ui.mjs` reports a region pattern that the catalog or the vocabulary does not declare. |
| standards/rule/frontend-ui.specify-pages-before-composition | inspection | `node standards/tools/validate-ui.mjs` reports a page specification with no sidecar or an unlisted shell, layout, or pattern. |
| standards/rule/frontend-ui.render-every-declared-state | static | `node standards/tools/validate-ui.mjs` reports a declared state that no component named by a region carries. |
| standards/rule/frontend-ui.keep-the-implementation-inside-the-frozen-plan | static | `node standards/tools/validate-ui.mjs` reports a `data-region` value in the route source that the sidecar does not name. |
| standards/rule/frontend-ui.place-acceptance-beside-the-route | static | `node standards/tools/validate-ui.mjs` reports an acceptance record outside the `evidence` directory of its route. |
| standards/rule/frontend-ui.resolve-every-acceptance-identifier | test | `node standards/tools/validate-ui.mjs` reports an `AC-` identifier with no criterion record or no runnable file. |
...
| standards/rule/frontend-ui.prove-ui-behavior-and-appearance | test | `node standards/tools/validate-ui.mjs` reports a vocabulary state that no evidence record covers. |
| standards/rule/frontend-ui.follow-the-agent-ui-protocol | inspection | Review confirms the sidecar was frozen before the implementation, and the acceptance run covers every identifier it names. |
...
| standards/rule/frontend-ui.promote-a-recipe-on-its-second-consumer | inspection | Review confirms each catalog recipe has two consumers or records the second consumer it expects. |
