...
This document defines the security baseline requirements for all Litenova Solutions projects. These rules are non-negotiable. Violations MUST be caught in code review before any PR is merged.
...
All external input — HTTP request bodies, query parameters, route values, file uploads — MUST be validated at the application boundary before it reaches the Application layer.
...
Never trust input because it "comes from our own frontend." Validate every HTTP request as if it could come from any source.
...
// BAD:
app.MapPost("/posts", HandleAsync)
    .RequireAuthorization("AuthenticatedUser"); // ← magic string, not refactoring-safe
```
...
All engineers working on Litenova Solutions projects should be familiar with the [OWASP Top 10](https://owasp.org/www-project-top-ten/) — the most critical security risks in web applications. The rules in this document address the most common of these risks, but they are not exhaustive.
