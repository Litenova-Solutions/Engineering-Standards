...
- The solution holds four production projects and no server project. (CLIENT.STRUCTURE.001)
- Features sit under their module and use-case names. (CLIENT.FEATURES.001)
- A module never reaches into another module's feature internals. (CLIENT.BOUNDARIES.001)
- Imports run from pages inward, never outward. (CLIENT.IMPORTS.001)
- Domain and Application carry no browser type. (CLIENT.DOMAIN.001)

## Standards

### Use the client solution tree (CLIENT.STRUCTURE.001)
...
### Organize features by module and use case (CLIENT.FEATURES.001)
...
### Isolate module internals (CLIENT.BOUNDARIES.001)
...
### Keep imports directional (CLIENT.IMPORTS.001)
...
### Keep Domain free of browser concerns (CLIENT.DOMAIN.001)
...
### Place interop adapters together (CLIENT.CONVENTION.001)
...
### Keep static content out of the assembly (CLIENT.CONVENTION.002)
...
| ID | Method | Evidence |
|:---|:---|:---|
| CLIENT.STRUCTURE.001 | test | `ClientSolutionTests` asserts four production projects exist and no server project is present. |
| CLIENT.FEATURES.001 | test | `ClientArchitectureTests` asserts each feature path resolves to a declared module and use case. |
| CLIENT.BOUNDARIES.001 | test | `ClientArchitectureTests` asserts no cross-module reference resolves an internal feature type. |
| CLIENT.IMPORTS.001 | test | `ClientArchitectureTests` asserts the project reference graph matches the declared direction. |
| CLIENT.DOMAIN.001 | test | `ClientArchitectureTests` asserts Domain and Application reference no browser or interop type. |
| CLIENT.CONVENTION.001 | inspection | Interop review locates each adapter and its matching script, or records a named local replacement. |
| CLIENT.CONVENTION.002 | inspection | Published output serves runtime content from `wwwroot/` rather than an embedded resource. |
