...
- A component renders from parameters and raises events. (CLIENTUI.PRESENTATION.001)
- Every parameter carries a concrete feature-owned type. (CLIENTUI.PARAMETERS.001)
- A child reports upward through a callback. (CLIENTUI.EVENTS.001)
- A view model exposes no Domain or Application type. (CLIENTUI.VIEWMODELS.001)
- Browser access runs through an Application interface. (CLIENTUI.INTEROP.001)
- One component per file, named for what it renders. (CLIENTUI.FILES.001)

## Standards

### Keep components presentational (CLIENTUI.PRESENTATION.001)
...
### Declare every parameter type (CLIENTUI.PARAMETERS.001)
...
### Raise events rather than mutating (CLIENTUI.EVENTS.001)
...
### Own the layer contract (CLIENTUI.VIEWMODELS.001)
...
### Resolve browser behavior through an interface (CLIENTUI.INTEROP.001)
...
### Place one component per file (CLIENTUI.FILES.001)
...
### Name for the boundary role (CLIENTUI.CONVENTION.001)
...
### Apply one styling system (CLIENTUI.CONVENTION.002)
...
### Keep accessibility in the component (CLIENTUI.CONVENTION.003)
...
| ID | Method | Evidence |
|:---|:---|:---|
| CLIENTUI.PRESENTATION.001 | test | `ClientArchitectureTests` asserts no component resolves storage, interop, or a Domain service. |
| CLIENTUI.PARAMETERS.001 | test | `ComponentContractTests` asserts each parameter declares a feature-owned concrete type. |
| CLIENTUI.EVENTS.001 | test | `ComponentContractTests` asserts each child reports upward through a callback parameter. |
| CLIENTUI.VIEWMODELS.001 | test | `ClientArchitectureTests` asserts no view model exposes a Domain or Application type. |
| CLIENTUI.INTEROP.001 | test | `ClientArchitectureTests` asserts no component injects the interop runtime directly. |
| CLIENTUI.FILES.001 | test | `ComponentContractTests` asserts each component file declares one primary component. |
| CLIENTUI.CONVENTION.001 | inspection | Naming review compares each feature component against its rendered role. |
| CLIENTUI.CONVENTION.002 | inspection | Style review locates each color, spacing, and font value in the token set. |
| CLIENTUI.CONVENTION.003 | test | `ComponentAccessibilityTests` covers keyboard paths and announcements for each interactive component. |
