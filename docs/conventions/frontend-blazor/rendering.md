...
- Rendering runs in the browser with no server round trip. (CLIENTRENDER.MODE.001)
- A routable component declares its own template under `Pages/`. (CLIENTRENDER.ROUTES.001)
- Each route defines loading, empty, error, and content behavior. (CLIENTRENDER.STATES.001)
- Pages a first-time visitor reads download no runtime. (CLIENTRENDER.ACQUISITION.001)
- The application starts and navigates offline after first load. (CLIENTRENDER.OFFLINE.001)

## Standards

### Render on the client only (CLIENTRENDER.MODE.001)
...
### Declare routes on pages (CLIENTRENDER.ROUTES.001)
...
### Give every route explicit states (CLIENTRENDER.STATES.001)
...
### Keep the acquisition surface static (CLIENTRENDER.ACQUISITION.001)
...
### Operate offline after first load (CLIENTRENDER.OFFLINE.001)
...
### Name routes for the reader (CLIENTRENDER.CONVENTION.001)
...
### Keep navigation state in the URL (CLIENTRENDER.CONVENTION.002)
...
| ID | Method | Evidence |
|:---|:---|:---|
| CLIENTRENDER.MODE.001 | test | `PublishOutputTests` asserts the output declares no render mode requiring a server. |
| CLIENTRENDER.ROUTES.001 | test | `RouteContractTests` asserts each routable component declares one template with typed parameters. |
| CLIENTRENDER.STATES.001 | test | `RouteStateTests` asserts each route renders its loading, empty, error, and content states. |
| CLIENTRENDER.ACQUISITION.001 | test | `AcquisitionTests` asserts a landing page loads without fetching the framework payload. |
| CLIENTRENDER.OFFLINE.001 | test | `OfflineStartupTests` starts the application with the network disabled after a first load. |
| CLIENTRENDER.CONVENTION.001 | inspection | Route review compares each template against its documented use-case name. |
| CLIENTRENDER.CONVENTION.002 | inspection | Navigation review confirms refresh-surviving state appears in the route or query string. |
