...
- Four test projects match the four client boundaries. (CLIENTTEST.PROJECTS.001)
- Every acceptance criterion of a verified use case is cited by a test. (CLIENTTEST.ACCEPTANCE.001)
- Domain and Application tests run with no browser or storage. (CLIENTTEST.CORE.001)
- Component tests substitute every Application interface. (CLIENTTEST.COMPONENTS.001)
- A browser suite covers the primary flow, storage loss, offline, and portability. (CLIENTTEST.E2E.001)
- CI fails when the first-load payload exceeds its budget. (CLIENTTEST.BUDGET.001)

## Standards

### Use the four client test projects (CLIENTTEST.PROJECTS.001)
...
### Cite acceptance criteria (CLIENTTEST.ACCEPTANCE.001)
...
### Test the core without a browser (CLIENTTEST.CORE.001)
...
### Replace interop in component tests (CLIENTTEST.COMPONENTS.001)
...
### Verify real browser behavior end to end (CLIENTTEST.E2E.001)
...
### Enforce the first-load budget (CLIENTTEST.BUDGET.001)
...
### Keep browser-capability tests honest (CLIENTTEST.CONVENTION.001)
...
### Keep end-to-end tests few (CLIENTTEST.CONVENTION.002)
...
| ID | Method | Evidence |
|:---|:---|:---|
| CLIENTTEST.PROJECTS.001 | test | `ClientSolutionTests` asserts the four test projects exist and run in CI. |
| CLIENTTEST.ACCEPTANCE.001 | static | `node standards/tools/validate-consumer.mjs` resolves each acceptance identifier to a test reference. |
| CLIENTTEST.CORE.001 | test | `ClientArchitectureTests` asserts no core test project references a browser or interop type. |
| CLIENTTEST.COMPONENTS.001 | test | `ComponentContractTests` asserts each render substitutes every Application interface. |
| CLIENTTEST.E2E.001 | test | `EndToEndTests` covers the primary flow, storage loss, offline startup, and export with import. |
| CLIENTTEST.BUDGET.001 | operation | The CI budget job compares published transfer size against the Operating Limits value. |
| CLIENTTEST.CONVENTION.001 | test | `CapabilityTests` covers the supported and unsupported path of each capability-dependent feature. |
| CLIENTTEST.CONVENTION.002 | inspection | Browser suite review confirms each case maps to a required flow or a named interaction. |
