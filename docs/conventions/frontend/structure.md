...
One module cannot import another module's internal feature path. Route composition may render public components from more than one module when a page specification requires it.
...
Create a module-local `shared/` folder only for code used by two use cases in that module. Cross-module primitives belong outside `features/`.
...
- Run lint rules that reject cross-module and cross-application imports.
