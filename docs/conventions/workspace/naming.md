...
- One primary type per C# file, named for the file. (NAME.FILE.001)
- Aggregate-owned types lead with their aggregate root name. (NAME.AGGREGATE.001)
- Architectural roles carry their declared suffix. (NAME.SUFFIX.001)
- Async methods end in Async and take the token last. (NAME.ASYNC.001)
- Exception names state the failed rule and its owner. (NAME.EXCEPTION.001)
- Boolean names read as conditions. (NAME.BOOLEAN.001)
- Production C# follows the declared style rules. (NAME.CSHARP.001)
- Code uses the pinned language version's current constructs. (NAME.CSHARP.002)
- Frontend names follow one predictable case mapping. (NAME.FRONTEND.001)
...
| ID | Method | Evidence |
|:---|:---|:---|
| NAME.FILE.001 | inspection | `NamingTests` asserts each C# file declares one primary top-level type matching its file name. |
| NAME.AGGREGATE.001 | inspection | `NamingTests` asserts each aggregate-owned type name begins with its aggregate root name. |
| NAME.SUFFIX.001 | inspection | `NamingTests` asserts each architectural type carries the suffix its role requires. |
| NAME.ASYNC.001 | static | `NamingTests` asserts each task-returning method ends in `Async` with the cancellation token last. |
| NAME.EXCEPTION.001 | static | `NamingTests` asserts each Domain exception name leads with its aggregate root and names its reason. |
| NAME.BOOLEAN.001 | inspection | `NamingTests` asserts each boolean member uses an intent-revealing prefix or verb. |
| NAME.CSHARP.001 | inspection | The Release build enforces the style rules through analyzer warnings promoted to errors. |
| NAME.CSHARP.002 | inspection | The Release build enforces the language version and modern-construct analyzers as errors. |
| NAME.FRONTEND.001 | inspection | `node standards/tools/validate-ui.mjs` reports a frontend path or export that breaks the naming form. |
| NAME.CONVENTION.001 | inspection | Module review compares each layer folder name against the module identifier. |
| NAME.CONVENTION.002 | inspection | `NamingTests` asserts each namespace matches its folder path beneath the project root. |
| NAME.CONVENTION.003 | inspection | `NamingTests` reports each generic boundary name for review against a narrower alternative. |
| NAME.CONVENTION.004 | inspection | Boundary review compares each route segment and field name against its current ubiquitous term. |
