...
- One folder per operation holds its message, result, validator, and handler. (APP.STRUCTURE.001)
- Commands and queries dispatch through their own mediator. (APP.MEDIATOR.001)
- Messages are public when hosts need them; handlers stay internal. (APP.CONTRACTS.001)
- Validators check input shape; Domain decides state permission. (APP.VALIDATION.001)
- Expected failures throw typed Application exceptions with stable codes. (APP.FAILURES.001)
- Handlers authorize against the target data they load. (APP.AUTHORIZATION.001)
- Command handlers load, call Domain, stage, and return. (APP.COMMAND.001)
- Queries project through the read session, never through aggregates. (APP.QUERY.001)
- Results mirror the Domain closed set rather than flattening it. (APP.CLOSEDSET.001)
- Ports name the business action and own their request and result types. (APP.PORTS.001)
...
| ID | Method | Evidence |
|:---|:---|:---|
| APP.STRUCTURE.001 | static | `ArchitectureTests` asserts each operation folder holds one message and its matching result, validator, and handler. |
| APP.MEDIATOR.001 | inspection | `ArchitectureTests` asserts no dispatch path resolves a shared bus abstraction over the two pinned mediators. |
| APP.CONTRACTS.001 | inspection | `ArchitectureTests` asserts handler and validator types are internal and sealed while messages and results carry their role suffix. |
| APP.VALIDATION.001 | inspection | `ValidationTests` asserts each validator rejects structural input and defers state decisions to the aggregate. |
| APP.FAILURES.001 | inspection | `UseCaseFailureTests` asserts each expected failure surfaces its stable code with no transport or provider detail attached. |
| APP.AUTHORIZATION.001 | inspection | `TargetAuthorizationTests` asserts each protected handler rejects a caller lacking the target grant it loads. |
| APP.COMMAND.001 | inspection | `ArchitectureTests` asserts no command handler commits a session, catches a domain exception, or references an HTTP type. |
| APP.QUERY.001 | inspection | `ArchitectureTests` asserts no query handler resolves a repository or returns an aggregate type. |
| APP.CLOSEDSET.001 | inspection | `ResultContractTests` asserts each result union carries one record per Domain case and exposes no Domain type. |
| APP.PORTS.001 | inspection | `ArchitectureTests` asserts no Application port signature names a provider type or transport model. |
| APP.REACTION.001 | inspection | `ReactionTests` asserts each reaction runs under the delivery classification its specification declares. |
| APP.ORCHESTRATION.001 | inspection | `ArchitectureTests` asserts no command handler resolves or calls the command mediator. |
| APP.WORKFLOW.001 | inspection | `WorkflowOrchestrationTests` asserts the orchestrator stages progress and its outgoing command in one transaction. |
| APP.CONVENTION.001 | operation | Folder review compares each operation path against its use-case identifier, or records a named local replacement. |
| APP.CONVENTION.002 | inspection | `ArchitectureTests` asserts each message and result is a record exposing no aggregate or Domain union type. |
| APP.CONVENTION.003 | inspection | `ArchitectureTests` asserts no Application result exposes an aggregate, session, provider, or HTTP type. |
| APP.CONVENTION.004 | inspection | Mapping review locates each conversion in the layer that owns its output shape. |
