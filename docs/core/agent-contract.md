...
Agents start with Tier 0 and use the manifest load plan for the active task. Tier 1 supplies short rules. Tier 2 supplies full detail. Recipes stay unloaded unless enabled.
...
Read `git status` before editing. Preserve user changes, untracked files, and unrelated generated output. Use minimal commits grouped by one reason for change.

## AGENT.COMPLETE.001 - Prove completion

Run every applicable check, inspect generated differences, and compare the result with the requested behavior. A passing unit test does not prove an untested deployment or documentation requirement.
