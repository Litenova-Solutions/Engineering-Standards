...
- Use compiler, schema, generator, and test checks for enforceable boundaries.
...
Write each rule, version, status, route, and acceptance criterion once. Other artifacts link to or generate from that source.

Package versions belong in `standards.manifest.json`. Use-case status belongs in its frontmatter. Test coverage is generated from acceptance IDs and test references.
...
Use project references, architecture tests, schema validation, generators, and CI for mechanical constraints. Keep prose for decisions, intent, and boundaries that require judgment.
