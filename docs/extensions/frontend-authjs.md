...
Keep Auth.js configuration under one server-owned module such as `auth.ts`. Keep provider-specific claim mapping near that configuration. Feature code consumes a project-owned session view rather than provider response objects.
