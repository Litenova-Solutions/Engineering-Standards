...
### Stage aggregate writes through repositories (BACKEND.PERSISTENCE.WRITE.001)
...
### Query through IQuerySession (BACKEND.PERSISTENCE.READ.001)
...
### Commit once in the command pipeline (BACKEND.PERSISTENCE.COMMIT.001)
...
### Stage Workflow progress with outgoing work (BACKEND.PERSISTENCE.WORKFLOW.001)
...
### Keep mappings and aliases explicit (BACKEND.PERSISTENCE.MAPPING.001)
...
### Keep serialization behavior out of Domain (BACKEND.PERSISTENCE.SERIALIZATION.001)
...
### Use database naming conventions (BACKEND.PERSISTENCE.NAMING.001)
...
### Use this Infrastructure layout (BACKEND.PERSISTENCE.CONVENTION.001)
...
### Register one scoped session (BACKEND.PERSISTENCE.CONVENTION.002)
...
### Make ordering explicit (BACKEND.PERSISTENCE.CONVENTION.003)
...
### Review query plans for new indexes (BACKEND.PERSISTENCE.CONVENTION.004)
...
### Add read documents for query-shaped data (BACKEND.PERSISTENCE.CONVENTION.005)
...
### Persist one explicit state object (BACKEND.PERSISTENCE.CONVENTION.006)
...
This informative example demonstrates `BACKEND.PERSISTENCE.SERIALIZATION.001` and `BACKEND.PERSISTENCE.CONVENTION.001`.
...
| BACKEND.PERSISTENCE.CONVENTION.001 | inspection | Folder review locates store configuration, repositories, and commit behavior under the persistence folder, or records a named local replacement. |
| BACKEND.PERSISTENCE.CONVENTION.002 | inspection | `SessionLifetimeTests` asserts repositories and the commit handler resolve one session per operation. |
| BACKEND.PERSISTENCE.CONVENTION.003 | inspection | `QueryOrderingTests` asserts each multi-item query declares ordering, a bound, and a tie-breaker. |
| BACKEND.PERSISTENCE.CONVENTION.004 | inspection | Index review records the accepted query and the PostgreSQL plan that justified it. |
| BACKEND.PERSISTENCE.CONVENTION.005 | inspection | Read-document review records its consistency and rebuild behavior alongside its owning use case. |
| BACKEND.PERSISTENCE.CONVENTION.006 | inspection | `SerializationTests` asserts each persisted aggregate round-trips its state record type. |
